...
async fn test_api_dynamodb_sam_pipeline() -> Result<()> {
    if !should_run() {
        eprintln!("Skipping api-dynamodb-sam E2E (set KONDUCTOR_E2E=1)");
        return Ok(());
    }

    let tmp = setup_test_dir(Some("api-dynamodb-sam"))?;
    let cwd = tmp.path();
    ...
    local
        .run_until(async {
            ...
            // Step 5: SAM + DynamoDB template checks
            ...
            assert!(
                content.contains("AWS::DynamoDB::Table")
                    || content.contains("AWS::Serverless::SimpleTable"),
                "template should contain a DynamoDB table resource"
            );

            if has_command("sam") {
                let (ok, output) = run_command(cwd, "sam", &["validate"]);
                if !ok {
                    eprintln!("[api-dynamodb-sam] sam validate failed (non-fatal): {output}");
                }
            }
